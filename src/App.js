...
function App({ uid }) {
  ...
  if (user === 'seller') {
    return (<Box bgcolor={'#070C12'} display={"flex"} flexDirection={'row'} maxHeight={'100vh'}>
      ...
      {selected === 0 ? <Main uid={uid}></Main> : selected === 1 ? <CollectionPage uid={uid}></CollectionPage> : selected === 2 ? <SellersAddCollections uid={uid}></SellersAddCollections> : <BuyersDataPage uid={uid}></BuyersDataPage>}
    </Box>);
  }else if (user === 'buyer'){
    return (
